...
public sealed partial class ConformanceSmokeTests
{
    ...
    internal static async Task<AggregateResult> RunConformanceSuiteAsync() => await RunConformanceSuiteAsync(IntegrationFixture.GetConformanceRunConfig());
    ...
    internal static async Task<AggregateResult> RunConformanceSuiteAsync(ConformanceRunConfig config)
    {
        ...
        {
            scenarios.Add(await RunScenarioAsync(scenario.ScenarioId, config.Transport, config.AuthMode));
        }
        ...
        await IntegrationFixture.WriteAggregateAsync(aggregate, config.OutputPath);
        return aggregate;
    }

    static async Task<ScenarioResult> RunScenarioAsync(string scenarioId, string transport, string authMode)
    {
        return scenarioId switch
        {
            "CS-001" => await RunCs001ConnectSuccess(transport, authMode),
            "CS-002" => await RunCs002AuthFailure(transport),
            "CS-003" => await RunCs003RequestSuccess(transport, authMode),
            "CS-004" => await RunCs004UnknownRoute(transport, authMode),
            "CS-005" => await RunCs005InvalidPayload(transport, authMode),
            "CS-006" => await RunCs006ServerErrorMapping(transport, authMode),
            "CS-007" => await RunCs007TimeoutHandling(transport, authMode),
            "CS-008" => await RunCs008CallerCancellation(transport, authMode),
            "CS-009" => await RunCs009DisconnectDuringRequest(transport, authMode),
            "CS-010" => await RunCs010ReconnectAndRetryBehavior(transport, authMode),
            "CS-011" => await RunCs011StreamReceiveSequence(transport, authMode),
            "CS-012" => await RunCs012StreamCompletion(transport, authMode),
            "CS-013" => await RunCs013StreamErrorMidFlight(transport, authMode),
            "CS-014" => await RunCs014ConcurrentInflightRequests(transport, authMode),
            "CS-015" => await RunCs015ShutdownDuringActiveWork(transport, authMode),
            "CS-017" => await RunCs017BoundedConcurrencyUnderBurstLoad(transport, authMode),
            "CS-016" => await RunCs016FilteredStreamReplay(transport, authMode),
