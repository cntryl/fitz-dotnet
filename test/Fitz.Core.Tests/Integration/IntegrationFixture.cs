...
static class IntegrationFixture
{
    ...
    internal static string GetConformanceClientName() => Environment.GetEnvironmentVariable("CONFORMANCE_CLIENT_NAME") ?? "fitz-dotnet";
    ...
    internal static Client CreateAnonymousClient(
        ...
        AsyncHandlerOptions? asyncHandlers = null) => CreateClient(url, transport, timeout, reconnect, tokenProvider: null, maxInFlightRequests, observability, asyncHandlers);
    ...
    static string NormalizeTransport(string? transport) => string.Equals(transport, "tcp", StringComparison.OrdinalIgnoreCase) ? "tcp" : "ws";

    static string CreateInvalidSignatureJwt(string secret, string audience) => CreateTestJwt($"{secret}-invalid", audience, GetJwtTenant(), DateTimeOffset.UtcNow.AddHours(1).ToUnixTimeSeconds());

    static string CreateTestJwt(string secret, string audience, long expiresAtSeconds) => CreateTestJwt(secret, audience, GetJwtTenant(), expiresAtSeconds);

    static string CreateTestJwt(string secret, string audience, string tenant, long expiresAtSeconds)
    {
    ...
    static string GetJwtTenant() => Environment.GetEnvironmentVariable("FITZ_BROKER_JWT_TENANT") ?? "dev";

    static string GetBrokerServiceName(string authMode)
    {
