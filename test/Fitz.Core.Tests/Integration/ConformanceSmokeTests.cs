...
public sealed partial class ConformanceSmokeTests
{
    ...
    static async Task<ScenarioResult> RunCs001ConnectSuccess(string transport, string authMode)
    {
        ...
        try
        {
            ...
            var tx = await client.Kv.BeginAsync(route, Cntryl.Fitz.KvDurability.Async);
    ...
    static async Task<ScenarioResult> RunCs002AuthFailure(string transport)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs003RequestSuccess(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            ...
            var readTx = await client.Kv.BeginAsync(route, Cntryl.Fitz.KvDurability.Async, KvMode.ReadOnly);
    ...
    static async Task<ScenarioResult> RunCs004UnknownRoute(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs005InvalidPayload(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            ...
            var tx2 = await client.Kv.BeginAsync(route, Cntryl.Fitz.KvDurability.Async);
            Exception? caught = null;
            try
            {
    ...
    static async Task<ScenarioResult> RunCs006ServerErrorMapping(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            ...
            var tx = await client.Kv.BeginAsync(kvRoute, Cntryl.Fitz.KvDurability.Async);
            await tx.InsertAsync("x"u8.ToArray(), "1"u8.ToArray());
            await tx.CommitAsync();

            var tx2 = await client.Kv.BeginAsync(kvRoute, Cntryl.Fitz.KvDurability.Async);
    ...
    static async Task<ScenarioResult> RunCs007TimeoutHandling(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            ...
            var kvRoute = IntegrationFixture.CreateUniqueRoute("kv");
            var tx = await callerClient.Kv.BeginAsync(kvRoute, Cntryl.Fitz.KvDurability.Async);
    ...
    static async Task<ScenarioResult> RunCs008CallerCancellation(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs009DisconnectDuringRequest(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs010ReconnectAndRetryBehavior(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            ...
            var kvRoute = IntegrationFixture.CreateUniqueRoute("kv");
            var kvTransaction = await client.Kv.BeginAsync(kvRoute, Cntryl.Fitz.KvDurability.Async);
            ...
            var reservedItems = await client.Queue.ReserveAsync(queueRoute, lease: TimeSpan.FromSeconds(30), batchSize: 1);
            var reservedItem = Assert.Single(reservedItems);

            var leaseRoute = IntegrationFixture.CreateUniqueRoute("lease");
            var lease = await client.Lease.AcquireAsync(leaseRoute, ttl: TimeSpan.FromSeconds(30));
            ...
            await AssertReconnectInvalidationAsync(
                ...
                evidence);
            await AssertReconnectInvalidationAsync(
                () => lease.ExtendAsync(TimeSpan.FromSeconds(30)),
            ...
            var postReconnectTx = await client.Kv.BeginAsync(postReconnectRoute, Cntryl.Fitz.KvDurability.Async);
    ...
    static async Task<ScenarioResult> RunCs011StreamReceiveSequence(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs012StreamCompletion(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs013StreamErrorMidFlight(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs014ConcurrentInflightRequests(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs015ShutdownDuringActiveWork(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            await client.ConnectAsync();
            var route = IntegrationFixture.CreateUniqueRoute("kv");
            var beginTask = client.Kv.BeginAsync(route, Cntryl.Fitz.KvDurability.Async);
    ...
    static async Task<ScenarioResult> RunCs017BoundedConcurrencyUnderBurstLoad(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
    ...
    static async Task<ScenarioResult> RunCs016FilteredStreamReplay(string transport, string authMode)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var evidence = new List<string>();
        ...
        try
        {
            ...
            if (!string.Equals(Encoding.UTF8.GetString(records[0].Body.Span), "alpha", StringComparison.Ordinal))
            {
                evidence.Add($"event-only read returned {Encoding.UTF8.GetString(records[0].Body.Span)}");
            ...
            if (!string.Equals(Encoding.UTF8.GetString(firstRecord.Body.Span), "alpha", StringComparison.Ordinal))
            {
                evidence.Add($"first page item body was {Encoding.UTF8.GetString(firstRecord.Body.Span)}");
    ...
    static ScenarioResult Result(string scenarioId, string transport, string authMode, string verdict, long latencyMs, IReadOnlyList<string> evidence, string notes = "", IReadOnlyDictionary<string, object?>? evidenceFields = null)
    {
    ...
    static async Task AssertReconnectInvalidationAsync(Func<Task> operation, Func<Exception, bool> predicate, string successEvidence, List<string> evidence)
    {
    ...
    static async Task<Exception?> CaptureExceptionAsync(Func<Task> operation)
    {
        try
        {
            await operation();
