...
| --- | --- | --- | --- | --- | --- |
| Conformance runner | `tests/Core/Integration/ConformanceSmokeTests.cs` and `ConformanceModels.cs` | Runner contract, `CS-001` to `CS-015` | Smoke-only runner executes `CS-001` to `CS-004`; aggregate fields are hardcoded to `websocket` and `anonymous` | `contract-drift` | No contract-compliant runner target yet |
| Transport matrix | `ClientConfig.Transport`, `TransportResolver.Resolve` | Suite required transports, `CS-001` to `CS-015` | WebSocket transport exists; non-WebSocket transport throws `NotSupportedException` | `partially implemented` | TCP support is missing |
| Connect and auth lifecycle | `IClient.ConnectAsync`, `Client`, `FitzConnection` | `AC-CONN-001` to `AC-CONN-005`, `CS-001`, `CS-002` | Fixed settle delay marks auth success; no explicit auth-success or auth-failure frame handling | `contract-drift` | Invalid JWT cannot reliably surface as typed auth failure |
| Reconnect contract | `ClientConfig`, `Client`, connection layer | `AC-CONN-006`, `CS-009`, `CS-010`, `CS-015` | No reconnect options, no backoff, no state rebuild after disconnect | `missing` | Public surface does not yet model reconnect behavior |
...
| Disconnect and shutdown behavior | `Client.DisposeAsync`, `FitzConnection.CloseAsync` | `CS-009`, `CS-015` | Connection close cancels receive loop and eventually cancels pending requests | `partially implemented` | No contract-level tests for shutdown during active work or reconnect follow-up |
...
- The largest parity blockers are the incomplete domain abstractions, auth-state correctness, missing TCP support, and the absence of a contract-compliant conformance runner.
