...
| --- | --- | --- | --- | --- |
| Shared conformance runner | `test/Fitz.Core.Tests/Integration/ConformanceSmokeTests.Runner.cs`, `ConformanceModels.cs`, `conformance/cross-language-conformance-suite.yaml` | `CS-001` to `CS-017` | `implemented` | Runner enforces suite completeness against the local YAML and emits the shared artifact schema |
...
| Heartbeat and silent transport loss handling | `HeartbeatOptions`, native TCP and WebSocket keepalive wiring | `CS-009`, reconnect runtime coverage | `implemented` | Native keepalive failures surface as reconnect-worthy transport failures; Fitz has no application heartbeat frame or client idle watchdog |
| Async handler execution controls | `AsyncHandlerOptions`, `AsyncHandlerDispatcher`, subscription and RPC worker dispatch | subscription and worker reconnect/runtime behavior | `implemented` | Handler concurrency is shared across subscriptions and RPC workers; callback timeout is optional and defaults to infinite |
| Observability primitives | `src/Fitz.Core/Observability/FitzObservability.cs`, lifecycle hooks in `FitzConnection` | lifecycle, retry, timeout, saturation evidence | `implemented` | Core stays dependency-light and exposes logger/tracer/meter/lifecycle callbacks |
