...
| MUX-1 | confirmed | The broker now advertises `CAP_CORRELATION` via `SERVER_HELLO` (4) and echoes a `CORRELATED` (3) record for any request labelled with `CORRELATE` (2). When it is advertised, requests carry a unique non-zero `u64` and are matched by identifier through an O(1) map with no lane, satisfying REQ-PROTO-021, REQ-PROTO-022 and REQ-CONC-009. Absent the advertisement the client still keeps one serialized lane per uncorrelated response message type, which REQ-CONC-010 makes a correctness requirement rather than a throughput one: the broker does not answer same-type requests in receive order. RPC `REQUEST`/`RESPONSE` keep their own 16-byte UUID and are never frame-correlated. Covered by `CorrelationTests` and `CorrelationConnectionTests`. |
