...
| CONN-3 | protocol-deferred | There is no Fitz application heartbeat frame. The misleading idle watchdog was removed; TCP keepalive remains enabled and WebSocket keepalive now configures native PING/PONG timeout detection from `HeartbeatOptions.Timeout`. |
| CONN-4 | confirmed | Outbound activity is recorded only after the transport send completes successfully. |
...
| MUX-7 | confirmed | Hot-path request and notification scans use allocation-free loops rather than LINQ. |
...
| BUF-5 | confirmed | Pooled writer, frame, TCP, and WebSocket buffers are returned with clearing; authentication token and encoded connect bytes are zeroed explicitly. |
| BUF-6 | confirmed | Malformed and truncated wire data now raises typed `ProtocolException` failures. |
| BUF-7 | intentionally accepted | The internal borrowed notification path remains for receive-path efficiency. Its lifetime is documented and asynchronous/public boundaries receive owned copies. |
...
| API-3 | confirmed | `ILease.FencingToken` is now a public immutable property, updated internally after successful renewal. |
| API-4 | confirmed | Renewal uses the lease lifecycle token plus a bounded per-operation deadline and cannot outlive disposal indefinitely. |
...
| API-7 | confirmed | Completed queue items dispose their disconnect listener after the successful wire operation. |
...
| API-10 | confirmed | Transaction, stream, lease, subscription, and worker disposal network operations are bounded and best-effort so cleanup cannot replace an exception already leaving a scope. |
...
| API-19 | confirmed | RPC sequence, terminal-error, and channel-completion transitions are correlated and serialized; no frame can be yielded after a terminal state. |
...
| API-22 | confirmed | Queue, KV, lease, stream, schedule, and RPC decoders validate counts, lengths, flags, and trailing bytes instead of accepting malformed defaults. |
| SUB-1 | confirmed | Each domain subscription table now uses one gate for read, mutation, unsubscribe, and restore transitions. |
...
| SUB-5 | confirmed | Subscription buffering uses configurable bounded channels instead of a fixed terminal capacity. Overflow terminates the affected subscription with a typed backpressure failure. |
...
| SUB-10 | intentionally accepted | One bounded client-wide async-handler queue is retained to enforce a predictable global resource ceiling. Per-subscription buffers isolate stream delivery and identify the subscription that overflowed. |
...
| SUB-12 | confirmed | Reconnect restore builds replacement registrations first and swaps subscription maps atomically from the dispatcher's perspective. |
...
| PKG-6 | rejected | Constant-route analyzer diagnostics intentionally remain errors: invalid constant routes are deterministic build-time defects. Runtime validation still covers dynamic routes. Analyzer rules resolve the actual Fitz interface symbols, accept stream read selectors, and are built against the minimum supported Roslyn 5.0 API surface. |
...
| PKG-9 | confirmed | Package metadata now includes author, license, repository, symbols, deterministic debug metadata, and SourceLink. |
...
Performance findings from the audit are represented above at `MUX-4`, `MUX-7`, `BUF-2`, `BUF-3`, `BUF-5`, `TR-6`, `API-9`, `SUB-3`, `SUB-5`, `SUB-6`, `SUB-10`, and `PKG-7`. The implementation uses bounded queues and channels, validates allocation sizes before renting or allocating, keeps pooled storage alive across asynchronous sends, clears sensitive/consumer data on return, and removes per-RPC-handler scans. Benchmark discovery and smoke execution are part of the verification record below.

## Verification record

- `dotnet build Fitz.sln -c Release --no-restore`: passed with zero warnings and zero errors.
- Separate Release runs of `Fitz.Core.Tests` and `Fitz.Analyzers.Tests` passed all 359 core tests and 11 analyzer tests, including the WebSocket conformance suite and its forced broker restart.
- `CONFORMANCE_TRANSPORT=tcp dotnet test test/Fitz.Core.Tests/Fitz.Core.Tests.csproj -c Release --no-build --filter 'FullyQualifiedName~Integration'`: all 30 TCP integration tests passed, including forced reconnect.
- The 329 non-integration core tests passed three consecutive runs with xUnit's normal parallel execution.
- `dotnet format Fitz.sln --no-restore --verify-no-changes --severity warn`: passed after standardizing all 300 test methods on PascalCase names.
...
- A clean NuGet-cache restore and execution of `Fitz.PackageConsumer` passed from the packed `0.1.3` artifacts and verified all three runtime assembly versions are `1.0.0.0`.
- The packed analyzer loaded and compiled valid stream-selector usage with SDK `10.0.109` / Roslyn 5.0 without CS9057.
- Native AOT publish for `osx-arm64` from only the packed packages succeeded, and the resulting executable ran successfully.
