...
static class RouteValidation
{
    internal static bool IsConcreteRoute(string route, string scheme) => TryValidateConcreteRoute(route, scheme, out _);

    internal static bool IsFixedRoute(string route, string scheme, int segmentCount) => TryValidateFixedRoute(route, scheme, segmentCount, out _);

    internal static bool IsSelectorRoute(string route, string scheme, int segmentCount, bool allowRealmWildcard) => TryValidateSelectorRoute(route, scheme, segmentCount, allowRealmWildcard, out _);

    internal static bool IsRegistrationPattern(string route, string scheme, int requiredSegments = 0) => TryValidateRegistrationPattern(route, scheme, requiredSegments, out _);

    internal static bool IsStreamSelector(string route) => TryGetStreamSelectorScope(route, out _);
    ...
    static string[] RouteSegments(string route)
    {
        var marker = route.IndexOf("://", StringComparison.Ordinal);
        return marker < 1 ? Array.Empty<string>() : route[(marker + 3)..].Split('/');
    }

    internal static bool TryValidateConcreteRoute(string route, string scheme, out RouteValidationFailure failure) => TryScan(route, scheme, expectedSegments: 0, allowWildcards: false, allowRealmWildcard: false, out _, out failure);

    internal static bool TryValidateFixedRoute(string route, string scheme, int segmentCount, out RouteValidationFailure failure) => TryScan(route, scheme, segmentCount, allowWildcards: false, allowRealmWildcard: false, out _, out failure);

    internal static bool TryValidateSelectorRoute(string route, string scheme, int segmentCount, bool allowRealmWildcard, out RouteValidationFailure failure) => TryScan(route, scheme, segmentCount, allowWildcards: true, allowRealmWildcard, out _, out failure);
    ...
    static bool TryGetPathStart(string route, string scheme, out int pathStart, out RouteValidationFailure failure)
    {
        pathStart = 0;
    ...
    static bool IsConcreteSegment(string route, int start, int length) => !IsWildcardSegment(route, start, length);
    ...
    static bool IsWildcardSegment(string route, int start, int length) => IsSingleWildcard(route, start, length) || IsDoubleWildcard(route, start, length);

    static bool IsSingleWildcard(string route, int start, int length) => length == 1 && route[start] == '*';

    static bool IsDoubleWildcard(string route, int start, int length) => length == 2 && route[start] == '*' && route[start + 1] == '*';
}
