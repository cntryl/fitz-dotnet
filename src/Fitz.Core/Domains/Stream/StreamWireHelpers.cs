...
static class StreamWireHelpers
{
    ...
    internal static void EnsureSuccessStatusOnly(ReadOnlyMemory<byte> response, string operation)
    {
        ...
        if (operation == "COMMIT")
            _ = ReadLengthPrefixedData(reader, operation);
        if (!reader.IsEof)
    ...
    static ReadOnlyMemory<byte> ReadLengthPrefixedData(BinaryBufferReader reader, string operation)
    {
        ...
        return reader.ReadMemory(length);
    }
    ...
    internal static StreamReadPage ReadReadPage(ReadOnlyMemory<byte> payload, string operation, string selector)
    {
        ...
        if (itemCount > int.MaxValue || itemCount > (uint)(reader.RemainingBytes / 5))
        {
