...
public sealed class StreamErrorEnvelopeTests
{
    ...
    public void should_preserve_versioned_domain_code_independently_of_wording(string operation, uint code, string message)
    {
        // Arrange
    ...
    public void should_preserve_legacy_error_envelopes(string operation, uint? code)
    {
        // Arrange
        using var writer = new BinaryBufferWriter();
        writer.WriteU8(1);
        if (code.HasValue) writer.WriteU32(code.Value);
