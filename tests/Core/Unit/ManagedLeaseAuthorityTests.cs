...
public sealed class ManagedLeaseAuthorityTests
{
    ...
    [InlineData((byte)0)]
    [InlineData((byte)1)]
    public async Task should_pass_exact_admission_authority_given_immediate_acquire(byte responseType)
    {
    ...
    [Fact]
    public async Task should_pass_final_authority_given_queued_acquire()
    {
    ...
    [Fact]
    public async Task should_keep_admission_authority_stable_given_renewal_rotates_live_token()
    {
    ...
    [Fact]
    public async Task should_not_invoke_authority_callback_given_failed_acquire()
    {
    ...
    [Fact]
    public async Task should_not_invoke_authority_callback_given_queued_acquire_times_out()
    {
        Action<byte[]>? acquireHandler = null;
        var callbackInvoked = false;
        using var leaseClient = new LeaseClient(
    ...
    [Fact]
    public async Task should_not_invoke_authority_callback_given_queued_acquire_is_cancelled()
    {
        var callbackInvoked = false;
    ...
    [Fact]
    public async Task should_preserve_one_argument_managed_lease_callbacks()
    {
    ...
    private static byte[] AcquireResponse(byte responseType, ulong token)
    {
        using var writer = new BinaryBufferWriter();
        writer.WriteU8(0);
    ...
    private static byte[] ErrorResponse(uint code, string message)
    {
        using var writer = new BinaryBufferWriter();
        writer.WriteU8(1);
